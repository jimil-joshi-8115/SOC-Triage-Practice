...
```
SOC-Triage-Practice/
├── Cases/                     → 22 closed triage cases (ticket → investigation → verdict)
├── Noise-Baselines/           → Documented known-legitimate patterns, distilled from all 20 cases
├── Triage-Playbooks/          → Step-by-step triage checklists by alert type, built from real case lessons
├── Metrics/                   → Full triage scorecard — accuracy, corrections, and key lessons across all cases
└── Quickfire-Simulator/       → Standalone interactive tool: procedurally-generated TP/FP/Ambiguous drills
```

---

## 📋 Case Index — 22 Cases Closed (Phase 4 In Progress: Cases 21-30)
...
| [Case_021](Cases/Case_021/) | Phase 4: Azure AD — Risky Sign-In + Legacy Auth MFA Bypass | 🔴 TP | T1078 | ✅ Closed |
| [Case_022](Cases/Case_022/) | Phase 4: Email/BEC — Phishing Report + Impossible Travel + Malicious Inbox Rule + Payment Fraud | Mixed (3 alerts) | T1078, T1114.003, T1114 | ✅ Closed |

**22 cases, 65 individual alerts triaged, spanning single-alert deep investigation (Phase 1), batch prioritization (Phase 2), full queue simulation under time pressure with live interrupts and cross-alert correlation (Phase 3, culminating in a 10-alert Final Exam), and expansion into new alert domains — Azure AD/cloud identity and Business Email Compromise — in Phase 4 (in progress).**
...
- **Phase 4 (Cases 21-30, in progress):** New alert domains — Azure AD/cloud identity, AWS, and Business Email Compromise — using realistic SOC-tool ticket formats (Microsoft Sentinel, Defender for Office 365). Cases are either ticket-only or Splunk-verified against realistic sample cloud log data, decided per case.

---

## Noise-Baselines & Triage-Playbooks

These two folders are the distilled output of the 20-case investigation work — not generic theory, but reference material built directly from confirmed verdicts across this repo, with every rule traceable back to the specific case that established it.
...
See [Metrics/triage_scorecard.md](Metrics/triage_scorecard.md) for the full case-by-case log: every verdict, every correction made and why, and the key lessons distilled from all cases closed so far.
